...
const Users = () => {
      ...
      if (response.ok) {
        ...
        setAlertConfig({
          ...
          message: isEditing 
            ...
            : `User account for @${formData.username} has been created.`
        });
      } else {
  ...
  // ... (executeRevoke and openConfirmModal remain the same) ...
  ...
  const executeRevoke = async () => {
    ...
    try {
      ...
      setAlertConfig({ show: true, type: 'error', title: "ERROR", message: "Failed to deactivate." });
    }
  };
  ...
  return (
    <div className="relative min-h-screen bg-slate-50 p-6 md:p-12 font-sans text-slate-900">
      ...
      {/* Confirmation Modal Render Logic Here */}
            ...
            <div className={`bg-white rounded-2xl border transition-all duration-300 shadow-sm overflow-hidden sticky top-8 ${isEditing ? 'border-indigo-400 ring-4 ring-indigo-50' : 'border-slate-200'}`}>
              <div className={`${isEditing ? 'bg-indigo-600 text-white' : 'bg-slate-50/50 text-slate-500'} px-6 py-4 border-b border-slate-200 flex justify-between items-center`}>
                <h2 className="text-[10px] font-black uppercase tracking-[0.2em]">
                  {isEditing ? 'Edit Existing Account' : 'Provision New Account'}
                </h2>
                {isEditing && (
                  <button onClick={cancelEdit} className="text-[9px] font-bold bg-white/20 hover:bg-white/30 px-2 py-1 rounded">CANCEL</button>
                )}
              </div>
                ...
                <form onSubmit={handleSubmit} className="space-y-5">
                  ...
                  <div className="space-y-1.5">
                    <label className="text-[10px] font-bold text-slate-400 uppercase tracking-widest ml-1">
                      {isEditing ? 'New Password (Optional)' : 'Create Password'}
                    </label>
                    <input
                      required={!isEditing}
                      type="password"
                  ...
                  <button
                    type="submit"
                    disabled={loading}
                    className={`w-full text-white py-4 rounded-xl text-[10px] font-bold uppercase tracking-widest transition-all shadow-xl active:scale-[0.98] disabled:opacity-50 mt-4 ${isEditing ? 'bg-indigo-600 hover:bg-indigo-700' : 'bg-slate-900 hover:bg-slate-800'}`}
                  >
                      ...
                      <tr key={u.id} className={`transition-colors group ${editingUserId === u.id ? 'bg-indigo-50/50' : 'hover:bg-slate-50/30'}`}>
                        ...
                        <td className="px-8 py-6 text-right">
                          <div className="flex justify-end gap-2 opacity-0 group-hover:opacity-100 transition-opacity">
                            <button
                              onClick={() => startEdit(u)}
                              className="bg-white text-indigo-600 border border-indigo-100 hover:bg-indigo-600 hover:text-white px-4 py-2 rounded-xl text-[10px] font-bold uppercase tracking-widest transition-all"
                            >
                              Edit
                            </button>
                            <button
                              onClick={() => setConfirmModal({ show: true, userId: u.id, username: u.username })}
                              className="bg-slate-100 text-slate-600 hover:bg-rose-600 hover:text-white px-4 py-2 rounded-xl text-[10px] font-bold uppercase tracking-widest transition-all"
                            >
                              Revoke
                            </button>
                          </div>
